...
namespace jm
{
	...
	void drawMymap(const RGB & color0)
	{
		setLineWidth(5.0f);
		drawLine(color0, vec2(-1.875f, -0.75f), color0, vec2(+1.875f, -0.75f)); //¸Ê 1Ãþ

		drawLine(color0, vec2(-1.875f, 0), color0, vec2(-1.0f, 0));//¸Ê 2Ãþ ¿ÞÂÊ
		drawLine(color0, vec2(+1.875f, 0), color0, vec2(+1.0f, 0));// ¸Ê 2Ãþ ¿À¸¥ÂÊ

		drawLine(color0, vec2(-1.875f, 0.5f), color0, vec2(-1.0f, 0.5f));//¸Ê 3Ãþ ¿ÞÂÊ
		drawLine(color0, vec2(+1.875f, 0.5f), color0, vec2(+1.0f, 0.5f));// ¸Ê 3Ãþ ¿À¸¥ÂÊ
	}
	...
	void drawLine(const RGB & color0,  vec2 & position0, vec2 & position1)
	{
		glBegin(GL_LINES);
		{
			glColor3fv(&color0[0]);
	...
	void rotate2(const float & degree, const float & scale_x, const float & scale_y)
	{
		glRotatef(degree, scale_x, 0.0, scale_y);
	}
	...
	void drawEnergybar(const RGB & color, const RGB & color2,const float & width, const float & height, int &energy,int &para)
	{
		if (para == 0)
		{
			float e = energy / 1000.0f;
			vec2 left_bottom(-0.5f * width, -0.5f * height);
			vec2 right(left_bottom.x + width * e, -0.5f * height);
			//const vec2 left_bottom_full(-0.5f * width, -0.3f * height);
			//vec2 const right_full(left_bottom.x + width , -0.3f * height);
			setLineWidth(9.0f);
			drawLine(color, left_bottom, right);
			setLineWidth(2.0f);
			//drawLine(color2, left_bottom_full, right_full);
			translate(0.0f, -0.012f);
			drawWiredBox(color2, width, height);
		}
		else if(para==1)
		{
			float e = energy / 100.0f;
			vec2 left_bottom(-0.5f * width, -0.5f * height);
			vec2 right(left_bottom.x + width * e, -0.5f * height);
			//const vec2 left_bottom_full(-0.5f * width, -0.3f * height);
			//vec2 const right_full(left_bottom.x + width , -0.3f * height);
